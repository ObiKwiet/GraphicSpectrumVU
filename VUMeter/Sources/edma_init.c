...
the "Active" buffer into a float buffer normalized to 1/-1.
****************************************************************/
void copyActiveToFloat(float fBuffer[]) {
    uint16_t i;
    for(i = 0; i < BUF_SIZE; i++) {
        fBuffer[i] = ((float)sampleBuffer[sBufferIdx.activeBuffer][i])/2048.0f - 1.0f;
...
void DMA1_IRQHandler(void)
{
    ...
    sBufferIdx.activeBuffer = ((sBufferIdx.activeBuffer + 2) % 3);
    sBufferIdx.lowerBuffer = ((sBufferIdx.lowerBuffer + 2) % 3);
    sBufferIdx.upperBuffer = ((sBufferIdx.upperBuffer + 2) % 3);
    ...
    EDMA_HAL_SetDmaRequestCmd(DMA_BASE_PTR, 0, 1);
		EDMA_HAL_SetDmaRequestCmd(DMA_BASE_PTR, 1, 1);
